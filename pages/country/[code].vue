  ...
  <div class="p-6 max-w-4xl mx-auto">
    ...
    <div v-if="country" class="bg-opacity-0 border-1 border-gray-800 rounded-xl shadow-md p-6 ">
      <div class="flex flex-col md:flex-row gap-6 items-center md:items-start">
        <img v-if="country.flags?.svg" :src="country.flags?.svg" :alt="country.flags?.alt" class="w-32 h-auto rounded-md shadow" />
        ...
        <div class="flex-1 space-y-3">
          ...
          <p><span class="font-semibold">Population :</span> {{ country.population?.toLocaleString('fr-FR') ?? 'N/A' }}</p>
          <p><span class="font-semibold">Langues :</span> {{ languages }}</p>
          <p><span class="font-semibold">Devise :</span> {{ currencies }}</p>
          <p><span class="font-semibold">Démonyme féminin :</span> {{ country.demonyms.fra?.f ?? 'N/A' }}</p>
          <p><span class="font-semibold">Démonyme masculin :</span> {{ country.demonyms.fra?.m ?? 'N/A' }}</p>
        </div>
      </div>
...
const country = computed(() =>
  store.countries.find(c => c.id.toLowerCase() === String(route.params.code).toLowerCase())
)
